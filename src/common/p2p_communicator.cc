...
void P2PCommunicator::Init(int world_size, void *ctx) {
  ...
  buffer_ = std::make_unique<PersistentBuffer>(buf_size * (world_size - 1));
  unsigned char *recv_bufs = static_cast<unsigned char*>(buffer_->RawPointer());
...
//    recv_comms.emplace(peer_rank, CommData(recv_bufs));
    send_comms.emplace(peer_rank, CommData(recv_bufs));
    recv_comms.emplace(peer_rank, CommData());
    recv_bufs += buf_size;
  }
...
int P2PCommunicator::TestRecv(int rank) {
  CommData &commData = recv_comms[rank];
  int flag;
