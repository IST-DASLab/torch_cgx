...
MaxMinEncodeValue(T input, T *feedback, unsigned char *meta_info, float rand) {
...
__device__ void find_meta_parallel(T *input, unsigned char *meta,
  ...
  extern __shared__ __align__(sizeof(T))
    ...
    if (VECTORIZE_DECOMPRESS) {
      ...
      if ((i + 1) * PACK_SIZE > num_elems) {
        for (unsigned int j = 0; j < num_elems - i * PACK_SIZE; j++) {
          ...
          T d = MaxMinDecodeValue < T > (encoded_value,
              meta_info,
              i * PACK_SIZE + j,
              bucket_size);
          if (ADD) {
...
void HIP_init_rand(RandState *states, int num_elems, unsigned int seed,
                   hipStream_t stream) {
  hipLaunchKernelGGL((_init_rand),
                     dim3(BLOCKS_PER_GRID(num_elems,
                                          THREADS_PER_BLOCK_COMPRESS)),
                     dim3(THREADS_PER_BLOCK_COMPRESS),
                     0,
                     stream,
                     seed,
                     states);
...
template<typename T>
void HIP_add(int n, const T *x, T *y, T *sum,
             hipStream_t stream) {
  int num_threads = umin(n, MAX_THREADS_PER_BLOCK);
  int blocks = BLOCKS_PER_GRID(n, num_threads);
  hipLaunchKernelGGL((_add<T>),
                     dim3(blocks), dim3(num_threads), 0, stream, n, x, y, sum);
...
inline void QUANTIZE(unsigned char *input_data, unsigned char *output_data,
                     ...
                     int bucket_size, RandState *states, hipStream_t stream,
                     int num_blocks, int num_threads,
                     int shared_memory_block_size) {
  switch (bits) {
    case 1:
      hipLaunchKernelGGL((quantize<T, EF, 1>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 2:
      hipLaunchKernelGGL((quantize<T, EF, 2>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 3:
      hipLaunchKernelGGL((quantize<T, EF, 3>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 4:
      hipLaunchKernelGGL((quantize<T, EF, 4>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 5:
      hipLaunchKernelGGL((quantize<T, EF, 5>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 6:
      hipLaunchKernelGGL((quantize<T, EF, 6>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 7:
      hipLaunchKernelGGL((quantize<T, EF, 7>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
      break;
    case 8:
      hipLaunchKernelGGL((quantize<T, EF, 8>),
                         dim3(num_blocks), dim3(num_threads),
                         shared_memory_block_size, stream, input_data,
                         output_data, feedback_data, num_elems, bucket_size,
                         states);
...
inline void DEQUANTIZE(unsigned char *input, unsigned char *meta_info,
                       ...
                       hipStream_t stream, int num_blocks,
                       int num_threads) {
  switch (bits) {
    case 1:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 1>),
                         dim3(num_blocks), dim3(num_threads), 0, stream, input,
                         meta_info, output, num_elems, bucket_size);
      break;
    case 2:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 2>),
                         dim3(num_blocks), dim3(num_threads), 0, stream, input,
                         meta_info, output, num_elems, bucket_size);
      break;
    case 3:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 3>),
                         dim3(num_blocks), dim3(num_threads), 0, stream, input,
                         meta_info, output, num_elems, bucket_size);
      break;
    case 4:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 4>),
                         dim3(num_blocks), dim3(num_threads), 0, stream, input,
                         meta_info, output, num_elems, bucket_size);
      break;
    case 5:
      ...
      break;
    case 6:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 6>),
                         dim3(num_blocks),
                         dim3(num_threads),
                         0,
                         stream,
      ...
      break;
    case 7:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 7>),
                         dim3(num_blocks),
                         dim3(num_threads),
                         0,
                         stream,
      ...
      break;
    case 8:
      hipLaunchKernelGGL((UnpackArray<T, ADD, 8>),
                         dim3(num_blocks),
                         dim3(num_threads),
                         0,
                         stream,
